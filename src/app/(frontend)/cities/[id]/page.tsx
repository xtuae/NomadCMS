...
export default function CityDetailsPage() {
  const { id } = useParams<{ id: string }>() // The dynamic route segment is `[id]`, so the parameter is `id`
  ...
  useEffect(() => {
    ...
    const loadCity = async () => {
      try {
        // Fetch all places with increased depth and filter client-side for more robust matching
        const apiUrl = `${process.env.NEXT_PUBLIC_PAYLOAD_URL}/places?limit=1000&depth=2` // Fetch a large number of documents with depth
        console.log('Fetching all cities from:', apiUrl) // Log the API URL
        ...
        if (!res.ok) {
          ...
          console.error(
            `Failed to fetch all places: ${res.status} ${res.statusText} - ${errorText}`,
          )
          throw new Error(`Failed to fetch all places`)
        }

        const data = await res.json()
        console.log('API response data (all places):', data) // Log the full API response
        console.log(
          'Country names from API:',
          data.docs.map((city: City) => city.countryName),
        ) // Log all country names

        const decodedId = decodeURIComponent(id).toLowerCase()
        console.log('Decoded URL ID for comparison:', decodedId)

        const foundCity = data.docs.find(
          (city: City) => city.countryName.trim().toLowerCase() === decodedId,
        )
        console.log('Found city after client-side filter:', foundCity)

        if (foundCity) {
          setCity(foundCity)
          // preset country in form for email
          setFormData((prev) => ({ ...prev, country: foundCity.countryName || '' }))
        } else {
          console.warn(`Place with countryName ${id} not found after client-side filtering.`)
          setCity(null) // Ensure city is null if not found
          throw new Error(`Place with countryName ${id} not found.`)
        }
      } catch (error) {
        console.error('Error fetching city:', error)
        setCity(null) // Ensure city is null on error
      }
    }
  ...
  return (
    <div className="city-details-container">
      <div className="sticky-header-wrapper">{/* remove if you don't have this component */}</div>
        ...
        <div className="quality-cont">
          ...
          <div className="quality-grid">
            ...
            <div className="rating-section">
              {[
                { label: '💰 Cost', value: city.costPerDay || 0, color: '#f97316' },
                {
                  label: '📶 Internet',
              ...
              <div className="steps-text">
                {(city.steps_apply || '')
                  ...
                  .map((line, index) => {
                    ...
                    return (
                      <div key={index} style={{ marginLeft: isIndented ? '20px' : '0' }}>
