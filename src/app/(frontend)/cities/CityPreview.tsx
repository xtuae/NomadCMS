...
export default function CityPreview() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_PAYLOAD_URL}/places?limit=100`, { cache: "no-store" });
    ...
    <div className="city-preview-wrapper">
      ...
      <section className="filter-section">
        <div className="filter-toolbar">
          ...
          />
          <select value={regionFilter} onChange={(e) => setRegionFilter(e.target.value)} className="filter-dropdown">
          ...
          <select value={visaFilter} onChange={(e) => setVisaFilter(e.target.value)} className="filter-dropdown">
